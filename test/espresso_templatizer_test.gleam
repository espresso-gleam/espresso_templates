...
import espresso_templatizer.{
  ...
  comment, document, documents, element, html_comment, text,
}
import nibble.{run}
...
pub fn self_closing_element_test() {
  let result =
    run(
      "<head>
         <meta lang=\"en\" />
  ...
  should.equal(
    ...
    Ok(Element(
      ...
      [
        ...
        Element(
          "link",
          [Attribute("rel", "stylesheet"), Attribute("href", "style.css")],
          [],
        ),
        Element("script", [Attribute("src", "test.js")], []),
      ],
    )),
  )
}
